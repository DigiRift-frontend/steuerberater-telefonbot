...
function buildCrmDescription(body: LeadPayload, ip: string): string {
  const lines: string[] = [];
  lines.push(`[steuerberater-telefonbot.de] ${body.type.toUpperCase()}`);
  lines.push("");

  if (body.praxisName) lines.push(`Kanzlei: ${body.praxisName}`);
  ...
  if (body.type === "quiz" && body.quizScore != null) {
    ...
    if (body.quizAnswers) {
      ...
      if (a.mandantenWert) lines.push(`Ø Mandantenwert: ${a.mandantenWert} €`);
    }
  }
...
function getCrmTags(body: LeadPayload): string[] {
  const tags: string[] = ["steuerberater-telefonbot.de"];
...
async function createCrmLead(body: LeadPayload, ip: string) {
  ...
  const lastName = nameParts.length > 1 ? nameParts.slice(1).join(" ") : "(steuerberater-telefonbot)";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // CRM only — no DigiLetter here (DOI flow via quiz-submit / ebook-submit)
